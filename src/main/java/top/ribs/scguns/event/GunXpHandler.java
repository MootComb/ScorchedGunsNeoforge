    ...
    public static void onPlayerXpPickup(PlayerXpEvent.PickupXp event) {
        ...
        if (mainHand.getItem() instanceof GunItem gunItem) {
            ...
            for (IAttachment.Type type : IAttachment.Type.values()) {
                ...
                int mendingLevel = player.registryAccess().registryOrThrow(Registries.ENCHANTMENT)
                        .getHolder(Enchantments.MENDING)
                        .map(holder -> EnchantmentHelper.getItemEnchantmentLevel(holder, attachmentStack))
                        .orElse(0);
