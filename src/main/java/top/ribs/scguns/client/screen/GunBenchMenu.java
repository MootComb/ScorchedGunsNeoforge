...
public class GunBenchMenu extends AbstractContainerMenu {
    ...
    private void attemptAutoCrafting() {
        containerAccess.execute((level, pos) -> {
            ...
            Optional<RecipeHolder<GunBenchRecipe>> recipeOptional = level.getRecipeManager()
                    .getAllRecipesFor(GunBenchRecipe.Type.INSTANCE)
                    .stream()
                    .filter(recipe -> recipe.id().equals(activeRecipeId))
                    .findFirst();
            ...
            GunBenchRecipe recipe = recipeOptional.get().value();
    ...
    public void slotsChanged(Container container) {
        containerAccess.execute((level, pos) -> {
            if (!level.isClientSide) {
                ...
                Optional<RecipeHolder<GunBenchRecipe>> recipe = level.getRecipeManager().getRecipeFor(GunBenchRecipe.Type.INSTANCE, new GunBenchRecipe.Input(craftingContainer), level);

                if (recipe.isPresent()) {
                    ItemStack result = recipe.get().value().assemble(new GunBenchRecipe.Input(craftingContainer), level.registryAccess());
    ...
    private void consumeIngredients() {
        containerAccess.execute((level, pos) -> {
            if (!level.isClientSide) {
                ...
                Optional<RecipeHolder<GunBenchRecipe>> recipeOptional = level.getRecipeManager().getRecipeFor(GunBenchRecipe.Type.INSTANCE, new GunBenchRecipe.Input(craftingContainer), level);
