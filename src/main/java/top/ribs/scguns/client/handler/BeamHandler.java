...
public class BeamHandler {
    ...
    public static float[] getBeamColorForWeapon(ItemStack heldItem, Gun modifiedGun) {
        ...
        float time = (Minecraft.getInstance().level.getGameTime() + Minecraft.getInstance().getTimer().getGameTimeDeltaPartialTick(false)) % 100;
    ...
    public static void onRenderLevel(RenderLevelStageEvent event) {
        ...
        setupRenderContext(poseStack, partialTick, player);

        renderActiveBeams(mc, partialTick, poseStack, bufferSource);
