...
public class GunRenderingHandler {
    ...
    private boolean equipProgressFieldsUnavailable;
    private boolean equipProgressWarningLogged;
    ...
    private void renderCooldownIndicator(Minecraft mc, Player player, ItemStack heldItem) {
        if (Config.CLIENT.display.cooldownIndicator.get()) {
            ...
            if (!gun.getGeneral().isAuto()) {
                float coolDown = player.getCooldowns().getCooldownPercent(heldItem.getItem(), mc.getTimer().getGameTimeDeltaPartialTick(false));
                if (coolDown > 0.0F) {
                    float scale = 3;
    ...
    public void onRenderOverlay(RenderHandEvent event) {
        ...
        CompoundTag heldTag = getStackTag(heldItem);
        if (heldTag != null) {
            if (heldTag.contains("Model", Tag.TAG_COMPOUND)) {
                overrideModel = ItemStack.parseOptional(Gun.builtInRegistryProvider(), heldTag.getCompound("Model"));
            }
        }
    ...
    public void applyWeaponScale(ItemStack heldItem, PoseStack stack) {
        CompoundTag compound = getStackTag(heldItem);
        if (compound != null) {
    ...
    private void renderAttachments(@Nullable LivingEntity entity, ItemDisplayContext display, ItemStack stack, PoseStack poseStack, MultiBufferSource renderTypeBuffer, int light, float partialTicks) {
        ...
        if (stack.getItem() instanceof GunItem) {
            ...
            CompoundTag gunTag = getStackTag(stack);
            CompoundTag attachments = gunTag != null ? gunTag.getCompound("Attachments") : new CompoundTag();
    ...
    private void renderReloadArm(PoseStack poseStack, MultiBufferSource buffer, int light, Gun modifiedGun, ItemStack stack, HumanoidArm hand, float translateX) {
        ...
        float reload = ((mc.player.tickCount - ReloadHandler.get().getStartReloadTick() + mc.getTimer().getGameTimeDeltaPartialTick(false)) % interval) / interval;
    ...
    private float getEquipProgress(float partialTicks) {
        if (this.equipProgressFieldsUnavailable) {
            return 0.0F;
        }
        if (this.equippedProgressMainHandField == null || this.prevEquippedProgressMainHandField == null) {
            try {
                this.equippedProgressMainHandField = this.findItemInHandRendererField("mainHandHeight", "f_109302_");
                this.prevEquippedProgressMainHandField = this.findItemInHandRendererField("oMainHandHeight", "f_109303_");
            } catch (NoSuchFieldException e) {
                this.equipProgressFieldsUnavailable = true;
                this.warnEquipProgressFallback(e);
                return 0.0F;
            }
        }
    ...
    private Field findItemInHandRendererField(String... names) throws NoSuchFieldException {
        ...
        throw lastException != null ? lastException : new NoSuchFieldException("No field names provided");
    }

    private void warnEquipProgressFallback(Exception e) {
        if (!this.equipProgressWarningLogged) {
            this.equipProgressWarningLogged = true;
            ScorchedGuns.LOGGER.warn("Unable to access ItemInHandRenderer main-hand equip progress; using safe first-person gun render fallback.", e);
        }
    }
