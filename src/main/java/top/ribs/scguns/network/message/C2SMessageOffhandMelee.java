...
{
    public static final StreamCodec<RegistryFriendlyByteBuf, C2SMessageOffhandMelee> STREAM_CODEC = StreamCodec.of(
            C2SMessageOffhandMelee::encode,
            C2SMessageOffhandMelee::decode
    );
    ...
    private static void encode(RegistryFriendlyByteBuf buffer, C2SMessageOffhandMelee message) {
        buffer.writeInt(message.targetId);
        buffer.writeFloat(message.x);
        buffer.writeFloat(message.y);
        buffer.writeFloat(message.z);
    }
    private static C2SMessageOffhandMelee decode(RegistryFriendlyByteBuf buffer) {
    ...
    public static void handle(C2SMessageOffhandMelee message, MessageContext context) {
        context.execute(() -> {
    ...
    private static void performOffhandMeleeAttack(ServerPlayer player, Entity target, ItemStack weapon, float attackStrength) {
    ...
    private static float getWeaponAttackSpeed(ItemStack weapon) {
        double attackSpeed = weapon.getAttributeModifiers().compute(4.0, net.minecraft.world.entity.EquipmentSlot.MAINHAND);
        return Math.max(0.1F, (float) attackSpeed);
    }

    private static float getWeaponDamage(ItemStack weapon) {
        return (float) weapon.getAttributeModifiers().compute(1.0, net.minecraft.world.entity.EquipmentSlot.MAINHAND);
    }
}
