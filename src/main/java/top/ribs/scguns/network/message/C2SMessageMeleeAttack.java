...
public class C2SMessageMeleeAttack
{
    public static final StreamCodec<RegistryFriendlyByteBuf, C2SMessageMeleeAttack> STREAM_CODEC = StreamCodec.of(
            C2SMessageMeleeAttack::encode,
            C2SMessageMeleeAttack::decode
    );

	private static final ScheduledExecutorService scheduler = Executors.newScheduledThreadPool(1);
	private static final int BANZAI_CHECK_INTERVAL_MS = 50;
	private static void encode(RegistryFriendlyByteBuf buffer, C2SMessageMeleeAttack message) {
		// No data to encode
	}
	private static C2SMessageMeleeAttack decode(RegistryFriendlyByteBuf buffer) {
	...
	private static void handleNormalMeleeAttack(ServerPlayer player) {
