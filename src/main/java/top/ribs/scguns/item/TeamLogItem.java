...
public class TeamLogItem extends Item {
    ...
    @Override
    public void appendHoverText(ItemStack stack, TooltipContext context, @NotNull List<Component> tooltip, @NotNull TooltipFlag flag) {
        CompoundTag tag = getCustomData(stack);
        if (!tag.isEmpty()) {
            if (tag.contains("Entities", Tag.TAG_LIST)) {
                ListTag listTag = tag.getList("Entities", Tag.TAG_COMPOUND);
    ...
    protected static void setCustomData(ItemStack stack, CompoundTag tag) {
        if (tag.isEmpty()) {
