...
public class TheMerchantEntity extends PathfinderMob implements Merchant {
    ...
    public void addAdditionalSaveData(CompoundTag compound) {
        ...
        for (MerchantOffer offer : this.offers) {
            offersTag.add(MerchantOffer.CODEC.encodeStart(NbtOps.INSTANCE, offer).getOrThrow());
        }
        ...
        if (compound.contains("Offers", Tag.TAG_LIST)) {
            ...
            for (int i = 0; i < offersTag.size(); i++) {
                try {
                    ...
                    MerchantOffer offer = MerchantOffer.CODEC.parse(NbtOps.INSTANCE, offerTag).getOrThrow();
